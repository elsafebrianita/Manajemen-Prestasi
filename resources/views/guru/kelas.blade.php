...
<head>
    ...
    <title>Kelas yang Diajar - SIMPRES</title>
        ...
        <div class="grid">
            @forelse($kelas as $k)
                <div class="card">
                    ...
                    <div class="info">
                        <h3>{{ $k->nama_kelas }}</h3>
                        <p>Jumlah Siswa: <strong>{{ $k->siswas()->count() }} Siswa</strong></p>
                    </div>
                </div>
            @empty
                <div class="card" style="grid-column: 1/-1; justify-content: center; padding: 40px; color: var(--text-muted);">
                    <i class="fas fa-info-circle"></i> Belum ada kelas yang diajar. Silakan hubungi admin.
                </div>
            @endforelse
        </div>
    </div>
