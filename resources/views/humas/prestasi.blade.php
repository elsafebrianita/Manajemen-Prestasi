@extends('layouts.app')
@section('page_title', 'Data Prestasi Publikasi')
    ...
    <div style="background: white; padding: 30px; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.05);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <div>
                <h2 style="color: var(--secondary); font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 22px; display: flex; align-items: center; gap: 10px;">
                    <i class="fa-solid fa-medal" style="color: var(--primary);"></i> Data Prestasi Siswa
                </h2>
                <p style="color: var(--text-muted); margin-top: 5px; font-size: 14px;">Seluruh data pengajuan prestasi siswa yang ada di sistem untuk referensi publikasi.</p>
            </div>
            <div>
                <form action="/humas/prestasi" method="GET" style="display: flex; gap: 10px;">
                    <select name="status" style="padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 14px; font-weight: 600; color: var(--secondary);">
                        <option value="">Semua Status</option>
                        <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Menunggu Validasi</option>
                        <option value="disetujui" {{ request('status') == 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                        <option value="ditolak" {{ request('status') == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                    <select name="kategori_id" style="padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 14px; font-weight: 600; color: var(--secondary);">
        ...
        @if(request('status') || request('kategori_id'))
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 12px 20px; border-radius: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; font-size: 14px; flex-wrap: wrap;">
                ...
                <a href="/humas/prestasi" style="color: var(--danger); text-decoration: none; margin-left: auto; font-weight: 600; font-size: 13px; display: flex; align-items: center; gap: 4px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: var(--bg-color); text-align: left;">
                        <th style="padding: 15px; border-radius: 10px 0 0 10px; font-size: 12px; text-transform: uppercase; color: var(--text-muted); font-weight: 700;">Tanggal</th>
                        <th style="padding: 15px; font-size: 12px; text-transform: uppercase; color: var(--text-muted); font-weight: 700;">Siswa</th>
                        <th style="padding: 15px; font-size: 12px; text-transform: uppercase; color: var(--text-muted); font-weight: 700;">Prestasi</th>
                        <th style="padding: 15px; font-size: 12px; text-transform: uppercase; color: var(--text-muted); font-weight: 700;">Tingkat & Juara</th>
                        <th style="padding: 15px; border-radius: 0 10px 10px 0; font-size: 12px; text-transform: uppercase; color: var(--text-muted); font-weight: 700;">Status</th>
                    </tr>
                </thead>
                <tbody>
